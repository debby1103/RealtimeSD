    ...
    if args["encoder"] == "bert":
        ...
        if args["tasc"]:
            ...
            optimiser = optim.Adam([
                ...
                {'params': model.output.parameters(), 'lr': 2e-4},
                {'params': model.attention.parameters(), 'lr':2e-4},
                {'params':model.tasc_mech.parameters(), 'lr':2e-4}
            ], amsgrad = True, weight_decay = 10e-5)
...
def evaluate_trained_bc_model(data):    
    ...
    for j in range(len(args["seeds"])):
     ...
     #   index_model = saved_models.index(current_model)
        ...
        model.load_state_dict(torch.load('./test_models2/lin_sst/bert_dot0.model', map_location=device))

...
class Model(nn.Module):
    ...
    def forward(self, input, lengths, retain_gradient = False, ig = int(1)):
        ...
        return last_layer.squeeze(0), self.weights,yhat
...
def train(model, training, development, loss_function, optimiser, run,epochs = 10, cutoff = True, save_folder  = None, cutoff_len = 2):
    ...
    for epoch in trange(epochs, desc = "run {}:".format(run+1), maxinterval = 0.1):
        ...
        for sentences, lengths, labels in training:
          ...
          #  print(sentences.size())
            features, weights,yhat =  model(sentences, lengths)
...
def test(model, loss_function, data):
    ...
    with torch.no_grad():
        ...
        for sentences, lengths, labels in data:
            ...
            features, weights,yhat =  model(sentences, lengths)
            if total is None:
                total = features.detach().cpu()
            else:
                total = torch.cat((total,features.detach().cpu()),dim=0)
         #   continue
            if len(yhat.shape) == 1:
            ...
            _, ind = torch.max(yhat, dim = 1)
            if preds is None:
                preds = ind.detach().cpu()
            else:
                preds = torch.cat((preds,ind.detach().cpu()),dim=0)
            predicted.extend(ind.cpu())
    
            actual.extend(labels.cpu().numpy())
            print(ind.detach().cpu().numpy(),"real",labels.cpu().numpy())
   
      #  results = classification_report(actual, predicted, output_dict = True)
        torch.save(total,"total_features_main.pt")
        torch.save(preds,"preds_main.pt")
        results = None
